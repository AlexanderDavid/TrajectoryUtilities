...
class SocialVAE(torch.nn.Module):
    ...
    class P_Z(torch.nn.Module):
        def __init__(self, hidden_dim_fy, embed_dim, z_dim):
            ...
            self.std = torch.nn.Sequential(
                torch.nn.Linear(embed_dim, z_dim), torch.nn.Softplus()
            )
    ...
    def __init__(self, horizon, ob_radius=2, hidden_dim=256):
        ...
        self.rnn_fx_init = torch.nn.Sequential(
            ...
            torch.nn.ReLU6(),
            torch.nn.Linear(
                hidden_dim_fx * self.rnn_fx.num_layers,
                hidden_dim_fx * self.rnn_fx.num_layers,
            ),
        )
    ...
    def forward(self, *args, **kwargs):
        ...
        try:
            n_predictions = (
                kwargs["n_predictions"] if "n_predictions" in kwargs else next(args)
            )
        except:
            n_predictions = 0
        ...
        if stochastic:
            pred = pred.view(pred.size(0), n_predictions, -1, pred.size(-1)).permute(
                1, 0, 2, 3
            )
        pred = pred + x[-1, ..., :2]
        if C < 3:
    ...
    def learn(self, x, y, neighbor=None, map=None):
        ...
        if y.size(0) != self.horizon:
            print(
                "[Warn] Unmatched sequence length in inference and generative model. ({} vs {})".format(
                    y.size(0), self.horizon
                )
            )
    ...
    def loss(self, err, kl):
        ...
        return {"loss": kl + rec, "rec": rec, "kl": kl}
    ...
    def FPC(y, n_samples):
        # y: S x L x 2
        goal = y[..., -1, :2]
        goal_ = SocialVAE.kmeans(n_samples, goal)
        dist = np.linalg.norm(
            goal_[:, np.newaxis, :2] - goal[np.newaxis, :, :2], axis=-1
        )
